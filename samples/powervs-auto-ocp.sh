...
for i in ${SERVICE_INSTANT_ID[@]}
do
        success=0
        export PVS_SERVICE_INSTANCE_ID=$i
		source $WORKSPACE/ocs-upi-kvm/scripts/helper/powervs/parameters.sh
        echo "Invoking ./create-ocp.sh in $PVS_REGION" | tee -a $LOGDIR/create-ocp-$i-$LOGDATE.log
        ./create-ocp.sh 2>&1 | tee -a $LOGDIR/create-ocp-$i-$LOGDATE.log
        if [ "$?" != 0 ]; then
                echo "Retrying ./create-ocp.sh in $PVS_REGION" | tee -a $LOGDIR/create-ocp-$i-$LOGDATE.log
                sleep 5m
                ...
                if [ "$?" != 0 ]; then
                        ...
                        if [ "$?" != 0 ]; then
                                echo "ERROR: cluster destroy failed.  Use cloud GUI to remove virtual instances from $PVS_REGION" | tee -a $LOGDIR/destroy-ocp-$i-$LOGDATE.log
                        fi
                        continue
                else
