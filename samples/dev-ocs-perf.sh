...
export WORKER_DESIRED_CPU=3                                     # ~20 fio pods + ceph on 4 workers based on WORKER_VOLUME_SIZE
export WORKER_DESIRED_MEM=64
...
function perf_test () {
	...
	run-ci -m "performance" --cluster-name ocstest --cluster-path $WORKSPACE \
		...
		--collect-logs \
		tests/e2e/performance/test_fio_benchmark.py::TestFIOBenchmark::test_fio_workload_simple[CephBlockPool-random]
